...
- For prompt changes: log the intent — what failure or behaviour the prompt was meant to fix — not just what changed.
- For any sprint-backed change: include the ticket ID in the commit body (`Closes: t-xxxx`). The ticket holds the full reasoning.
...
- Before proposing a plan, catalog what already exists: related files, similar patterns, prior implementations.
...
- Keep AGENTS.md concise — only rules, gotchas, non-obvious conventions.
